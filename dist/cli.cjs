...
var anthropic = new import_sdk.Anthropic({ apiKey });
...
function findAllThJsonFolders(baseDir) {
  ...
  function recurse(currentPath) {
    try {
      const items = import_fs.default.readdirSync(currentPath, { withFileTypes: true });
      const hasTh = items.some((i) => i.isFile() && i.name === "th.json");
      if (hasTh) {
        results.push(currentPath);
      }
      items.filter((i) => i.isDirectory()).forEach((dir) => recurse(import_path.default.join(currentPath, dir.name)));
    } catch (error) {
...
function pathToIdentifier(fullPath) {
  const relativePath = fullPath.replace(TRANSLATE_ROOT + import_path.default.sep, "");
...
async function processTranslations() {
  ...
  if (!import_fs.default.existsSync(TRANSLATE_ROOT)) {
  ...
  for (const folder of folders) {
    ...
    const thPath = import_path.default.join(folder, "th.json");
    console.log(`
\u{1F504} Processing: ${identifier}`);
    try {
      const thJSON = JSON.parse(import_fs.default.readFileSync(thPath, "utf-8"));
      ...
      for (const lang of TARGET_LANGS) {
        const langPath = import_path.default.join(folder, `${lang}.json`);
        let existingFlatEntries = [];
        let keysToTranslate = [];
        let keysToDelete = [];
        if (import_fs.default.existsSync(langPath)) {
          try {
            const existingJSON = JSON.parse(import_fs.default.readFileSync(langPath, "utf-8"));
